...
os.chdir(os.path.dirname(__file__)) # se puede dejar comentada o descomentada, depende de si funciona

df = pd.read_csv('data/Advertising.csv', index_col=0)
df.columns = [col.lower() for col in df.columns] # las ponemos en minusculas

X = df.drop(columns=['sales']) # features
y = df['sales'] # target

X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.20, random_state=42) # hacemos split

numeric_features = ['tv', 'radio', 'newspaper']

numeric_pipeline = Pipeline([ # nos hacemos un pequeño pipeline para el preprocesamiento de las variables numéricas
...
model.fit(X, y) # aqui entrena con todo, hasta con el test, para luego guardarlo y usarlo en producción
